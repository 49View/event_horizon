sudo apt-get update
sudo apt-get install -y libssl-dev apt-transport-https libglew-dev libglfw3 libglfw3-dev libglu1-mesa-dev freeglut3-dev mesa-common-dev libopenal-dev libstdc++-9-dev libbz2-dev libjpeg-dev libpng-dev libtiff-dev libwebp-dev nettle-dev gnutls-dev libicu-dev libgtk2.0-dev npm
...
sudo gitbuild.sh opencv 4.2.0 opencv "-DWITH_OPENEXR=FALSE -DBUILD_JAVA=FALSE -DBUILD_TESTS=FALSE -DBUILD_EXAMPLES=FALSE" /usr/local
