...
//        loader.WriteGltfSceneToFile(gltfScene.model.get(), _key, true, true, true, false);
    ...
    void save( SceneGraph &sg, GeomSP asset ) {
        ...
        bufferViewIndex.buffer = 0;
        bufferViewIndex.byteLength = indexData.size();
        bufferViewIndex.byteOffset = 0;
        bufferViewIndex.target = 34963;

        bufferViewSoa.buffer = 0;
        bufferViewSoa.byteLength = posData.size();
        bufferViewSoa.byteOffset = bufferViewIndex.byteLength;
        bufferViewSoa.target = 34962;
        ...
        indexAccessor.count = vData->numIndices();
        indexAccessor.type = TINYGLTF_TYPE_SCALAR;

        posAccessor.bufferView = 1;
        posAccessor.byteOffset = 0;
        posAccessor.componentType = TINYGLTF_COMPONENT_TYPE_FLOAT;
        posAccessor.count = vData->numIndices();
        posAccessor.minValues = {vData->getMin()[0],vData->getMin()[1],vData->getMin()[2]};
        posAccessor.maxValues = {vData->getMax()[0],vData->getMax()[1],vData->getMax()[2]};
        ...
        meshPrimitive.attributes.emplace("POSITION", 1);
