...
static inline std::string& capitalizeStart( std::string& _input ) {
    if ( _input.empty() ) return _input;
    std::transform(_input.begin(), _input.begin() + 1, _input.begin(), ::toupper);
    return _input;
}

static inline std::string truncate( const std::string& s, int size ) {
    if ( size <= 0 || s.size() <= static_cast<size_t>( size ) ) return s;

    return s.substr(0, size);
}

static inline std::string arrayToStringCompact( const std::vector<std::string>& stringArray, int truncateAfter = -1 ) {

    std::string ret{};
    for ( auto elem : stringArray ) {
        ret += capitalizeStart(elem);
    }

    return truncate(ret, truncateAfter);
}
