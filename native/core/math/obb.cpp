...
double RotatingCalipers::_distance(const V2f &start, const V2f &end, const V2f &point)
{
    double dist = (point.y() - start.y()) * (end.x() - start.x()) - (point.x() - start.x()) * (end.y() - start.y());
    return dist;
}

std::vector<V2f> RotatingCalipers::convexHull(std::vector<V2f> P)
{
    size_t n = P.size();
    size_t k = 0;
    if (n <= 3) return P;
...
[[maybe_unused]] double RotatingCalipers::diameter(const std::vector<V2f> &p)
{
    std::vector<V2f> h = convexHull(p);
    size_t m = h.size();
    if (1 == m) return 0;
...
MinAreaRect RotatingCalipers::minAreaRect(const std::vector<V2f> &p)
{
    std::vector<V2f> points = convexHull(p);
...
//        cout << "(" << points[i].x() << "," << points[i].y() << ")," << endl;
    ...
    V2f out0(px, py);
    V2f out1(A1 * min_area_state.width, B1 * min_area_state.width);
    V2f out2(A2 * min_area_state.height, B2 * min_area_state.height);
    ...
    min_area_rect.rect.emplace_back(out0.x() + out1.x(), out0.y() + out1.y());
    min_area_rect.rect.emplace_back(out0.x() + out1.x() + out2.x(), out0.y() + out1.y() + out2.y());
    min_area_rect.rect.emplace_back(out0.x() + out2.x(), out0.y() + out2.y());

    min_area_rect.center = { out0.x() + (out1.x() + out2.x()) * 0.5f, out0.y() + (out1.y() + out2.y()) * 0.5f};
...
double RotatingCalipers::cross( const V2f& O, const V2f& A, const V2f& B ) {
    return (A.x() - O.x()) * (B.y() - O.y()) - (A.y() - O.y()) * (B.x() - O.x());
}

double RotatingCalipers::area( const V2f& a, const V2f& b, const V2f& c ) {
    return fabs((b.x() - a.x()) * (c.y() - a.y()) - (b.y() - a.y()) * (c.x() - a.x()));
}

double RotatingCalipers::dist( const V2f& a, const V2f& b ) {
    return hypot(a.x() - b.x(), a.y() - b.y());
}

std::ostream& operator<<( std::ostream& os, const MinAreaRect& rect ) {
    os << "width: " << rect.width << " height: " << rect.height << " area: " << rect.area << " angle_width: "
       << rect.angle_width << " angle_height: " << rect.angle_height << " center: " << rect.center << " corner: "
       << rect.corner << " vector_width: " << rect.vector_width << " vector_height: " << rect.vector_height;
    return os;
}
