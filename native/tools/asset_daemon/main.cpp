...
//            auto cmdThumbnail = "convert " +  getDaemonRoot() + dfs.entity.source + " -resize " + resizeString + " " +  getDaemonRoot() + dfs.entity.source;
//            auto ret = std::system( cmdThumbnail.c_str());
...
std::string getGeomElaborateCommand( DaemonFileStruct2& dfs, const std::string& filenameglb, const std::string& ftype ) {
...
SerializableContainer elaborateGeomFile( DaemonFileStruct2& dfs, const SerializableContainer &filedata, const std::string& ftype ) {
