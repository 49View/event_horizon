...
void Framebuffer::attachColorBuffer( unsigned int index ) {
    ...
    if ( mMultisample ) {
        GLCALL(glRenderbufferStorageMultisample(GL_RENDERBUFFER, WH::getMultiSampleCount(),
                                                pixelFormatToGlInternalFormat(mFormat), mWidth, mHeight));
    } else {
...
//    GLCALL( glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor));
    GLCALL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, mRenderToTexture->getHandle(),
                                  0));
