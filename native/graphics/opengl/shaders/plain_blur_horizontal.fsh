...
void main()
{
    ...
    vec3 result = texture2D(colorFBTexture, v_texCoord).rgb * weight[0]; // current fragment's contribution

    for(int i = 1; i < 5; ++i)
    {
        result += texture2D(colorFBTexture, v_texCoord + vec2(tex_offset.x * float(i), 0.0)).rgb * weight[i];
        result += texture2D(colorFBTexture, v_texCoord - vec2(tex_offset.x * float(i), 0.0)).rgb * weight[i];
    }
