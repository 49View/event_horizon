...
//    boost::shared_mutex cacheMutex; // cache mutex
	...
	// triangulation functions
	Mesh GetGlyphMesh( ushort glyphIndex ) const; // gets the glyph's mesh
...
public:
	...
	// glyph triangulation
	const Mesh& GetTriangulation( CodePoint ) const;

	//
	// returns a triangulated mesh for the given glyphIndex
	size_t GetTriCount( CodePoint ) const;                                                                          //
	// returns the number of tris in the triangulated mesh for the given code point
