...
void Matrix4f::setOrthogonalProjection( float leftv, float rightv, float topv, float bottomv, float nearv, float farv ) {
	...
	mRows[3] = Vector4f( -(( rightv + leftv ) / ( rightv - leftv )), -(( topv + bottomv ) / ( topv - bottomv )), -(( farv + nearv ) / ( farv - nearv )), 1.0f );
}
...
void Matrix4f::setFromRTS( const Vector3f& pos, const Vector3f& axis, const Vector3f& zoom, MatrixRotationOrder mro /*= MatrixRotationOrder::xyz*/ ) {
