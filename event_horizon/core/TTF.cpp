...
float Triangulator::GetTriSign( const std::vector<vec2f>& _verts ) const {
	// get vertices
	vec2f p0 = *( _verts.end() - 3 );
	vec2f p1 = *( _verts.end() - 2 );
	vec2f p2 = *( _verts.end() - 1 );
...
//    ulong length       =
	ReadULONG( itr );   // byte length of this subtable (including the header)
...
vec2f Font::GetKerning( CodePoint cp0, CodePoint cp1, bool hk ) const {
	...
	if ( version == 0 ) {
		...
		//        ushort versionLo =
		ReadUSHORT( itr ); // apple uses a different header, with the version being 4 bytes, so this is the last 2 bytes of version
...
float Font::ParseKernTableF0( FItr itr, ushort g0, ushort g1 ) const {
	...
	//    ushort searchRange   =
	ReadUSHORT( itr ); // The largest power of two less than or equal to the value of nPairs, multiplied by the size in bytes of an entry in the table.
//    ushort entrySelector =
	ReadUSHORT( itr ); // number of iterations required
//    ushort rangeShift    =
	ReadUSHORT( itr );
