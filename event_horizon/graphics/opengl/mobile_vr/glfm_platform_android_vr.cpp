...
static ovrUniform ProgramUniforms[] =
        {
                ...
                { ovrUniform::UNIFORM_VIEW_ID,				ovrUniform::UNIFORM_TYPE_INT,       "ViewID"		},
                { ovrUniform::UNIFORM_SCENE_MATRICES,		ovrUniform::UNIFORM_TYPE_BUFFER,	"SceneMatrices" },
        };
...
static bool ovrProgram_Create( ovrProgram * program, const char * vertexSource, const char * fragmentSource, const bool useMultiview )
{
    ...
    for ( int i = 0; i < sizeof( ProgramUniforms ) / sizeof( ProgramUniforms[0] ); i++ )
    {
        ...
        if ( ProgramUniforms[i].type == ovrUniform::UNIFORM_TYPE_BUFFER )
        {
                                                    ...
                                                    const ovrTracking2 * tracking, ovrMobile * ovr )
{
    ...
    for ( int eye = 0; eye < renderer->NumBuffers; eye++ )
    {
        ...
        GL( glBindBufferBase( GL_UNIFORM_BUFFER, scene->Program.UniformBinding[ovrUniform::UNIFORM_SCENE_MATRICES], scene->SceneMatrices ) );
        if ( scene->Program.UniformLocation[ovrUniform::UNIFORM_VIEW_ID] >= 0 )  // NOTE: will not be present when multiview path is enabled.
        {
            GL( glUniform1i( scene->Program.UniformLocation[ovrUniform::UNIFORM_VIEW_ID], eye ) );
        }
