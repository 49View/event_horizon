...
//	std::vector<std::vector<cv::Point>> contours = contoursSource;
	...
	for ( size_t i = 0; i < contoursSource.size(); i++  ) {
		double le = 0;
		for ( size_t m = 0; m < contoursSource[i].size()-1; m++  ) {
			le += cv::norm( contoursSource[i][m] - contoursSource[i][m+1] );
		}
	...
	for ( size_t i = 0; i < contoursSource.size(); i++  ) {
		...
		contours.push_back( {} );
		cv::approxPolyDP( cv::Mat( contoursSource[i] ), contours[contours.size()-1], cv::arcLength( cv::Mat( contoursSource[i] ), true )*0.005, true );
	}
...
void convertContoursArrayToFloats( const std::vector<std::vector<cv::Point2i> >& contoursi, std::vector<std::vector<Vector2f> >& contours ) {
...
void convertContoursArrayToInt( const std::vector<std::vector<Vector2f> >& contoursi, std::vector<std::vector<cv::Point2i> >& contours ) {
	for ( unsigned long i = 0; i < contoursi.size(); i++ ) {
		if ( contoursi[i].size() < 4 ) continue;
